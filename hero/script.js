...
function showMode() {
  modalBody.style.fontSize = "2.2em";
	modalBody.innerHTML = song.formatted + "<br /><br /><button class='mode-button easy'>Facil</button><br /><button class='mode-button hard'>Dificil</button>";
...
function showStartButton() {
  if (mode === "Easy") {
    song.keysEasy.forEach(function(keyArray){
    	const newArray = keyArray.map(number => ((6000 / song.tempo) * (number - 1)))
  ...
  modalBody.innerHTML = "<button class='start-button third'>S T A R T</button><br/><br/>PRESIONA ESPACIO o ENTER";
...
function gameEnd() {
  ...
  return firebase.database().ref("/scores/" + song.name + "/" + mode).once("value").then(function(snapshot) {
    ...
    var scoreHtml = "Mejores Puntuaciones:<br /><br />";
    var scoreCounter = 0;
    ...
    var topText = "";
    if (highScore) {
      topText += "Tu puntuacion mas alta es: " + score + " puntos!<br />";
      topText += "<input type='text' placeholder='Name' id='score-input'><br />";
      topText += "<button id='score-button'>Enviar puntuación más alta!</button><br /><br />";
    } else {
      topText += "Tu Puntuacion fue: " + score + "<br /><br />";
    }
    var bottomText = "<br />" + song.formatted + "<br />" + mode + "<br /><br /><a href='index.html'>Jugar De Nuevo</a><br /><a href='../index.html'>Menu Principal</a>";
